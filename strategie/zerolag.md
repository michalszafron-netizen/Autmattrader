...
// Stop Loss — ATR-based with a floor to prevent stops that are too tight
...
// Trailing Stop — moves SL to breakeven + offset after price travels X × SL distance
...
// ══════════════════════════════════════════════════════════════════════════════
// SL = max(ATR-based, minimum % floor) — prevents dangerously tight stops
...
// Persistent state for trailing stop
...
// ── TRAILING STOP ───────────────────────────────────────────────────────────
// After price travels trail_trigger × SL distance in profit, move SL to
// breakeven + offset (locks in guaranteed profit)
...
// Big arrows — all raw trend flips
...
// SL / TP lines — only visible while in a position
...
// Background flash on trailing activation
bgcolor(trail_active and strategy.position_size != 0 ? color.new(color.yellow, 93) : na, title = "Trail Active")

// ══════════════════════════════════════════════════════════════════════════════
// TABLE 1 — MTF TREND (top-right) — updates every bar, no barstate.islast
// ══════════════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════════════
// TABLE 2 — VOLATILITY CLUSTER (bottom-right)
// ══════════════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════════════
// TABLE 3 — POSITION STATUS (bottom-left) — shows exactly what is happening
// ══════════════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════════════
if long_signal
    alert("LONG entry at " + str.format("{0,number,#.##}", close) +
          " | SL: " + str.format("{0,number,#.##}", close - f_sl_dist(close)) +
          " | TP: " + str.format("{0,number,#.##}", close + f_sl_dist(close) * tp_rr),
          alert.freq_once_per_bar_close)

if short_signal
    alert("SHORT entry at " + str.format("{0,number,#.##}", close) +
          " | SL: " + str.format("{0,number,#.##}", close + f_sl_dist(close)) +
          " | TP: " + str.format("{0,number,#.##}", close - f_sl_dist(close) * tp_rr),
          alert.freq_once_per_bar_close)

if trail_active and not trail_active[1]
    alert("TRAILING activated — SL moved to " + str.format("{0,number,#.##}", sl_level),
          alert.freq_once_per_bar_close)
